...
export const updateAndSyncCart = async (
  userData,
  ...
  const formatCartData = await serverCartItems.map((pro, i) => {
    return {product: pro?.product?._id, quantity: pro?.quantity};
  });
...
export const getCurrentCart = async userData => {
  ...
  if (userData?.length) {
    if (localStorageCartItems.length) {
      const updateServerCartItems = await updateAndSyncCart(
        userData,
        localStorageCartItems,
        serverCartItems,
      );
...
export const updatedAndFormatCart = async (product, isExist, userCart) => {
  ...
  let formatCartData;
  if (isExist) {
    updatedCart = await userCart?.map(item =>
      item?.product?._id === data._id
        ? {...item, quantity: item.quantity + 1}
        : item,
    );
    formatCartData = await updatedCart?.map(item => {
...
export const addToCart = async (userData, product, isExist, userCart) => {
  const {updatedCart, formatCartData} = await updatedAndFormatCart(
    product,
    isExist,
    userCart,
  );
...
export const increaseQuantity = async (userData, userCart, isExist, id) => {
  ...
  if (userData?.length) {
    ...
    const formatCartData = await updatedCart?.map((item, i) => {
      return {product: item?.product?._id, quantity: item?.quantity};
    });
    const cartData = await addCartServer(userData, formatCartData);
    return cartData;
...
export const createOrder = async (userCart, userData) => {
  const devideId = await getFromStorage('deviceId');
  let headerObj = {
    deviceid: devideId,
    devicename: 'Android',
  ...
  console.log(userCart,'usercart create')
  const response = await serverRequest(`api/v1/private/orderCreate`,'POST',{products: userCart},headerObj);
...
export const dataBaseOrderCreate= async(userData,order_id,products,addressId)=>{
  ...
  const response = await serverRequest(`api/v1/private/order`,'POST',{order_id: order_id, products: products,addressId: addressId},headerObj);
  if(response?.success) {
